  ...
  return (
    <Card className="w-full max-w-2xl mx-auto shadow-lg">
      ...
      {state.learningPaths && state.success && (
        <div className="mt-6 p-6 border-t border-green-200 bg-green-50 rounded-b-lg">
          <h3 className="text-xl font-semibold text-green-700 mb-3 flex items-center">
            <CheckCircle className="h-6 w-6 mr-2 text-green-600" />
            Сгенерированный План Обучения:
          </h3>
          <pre className="whitespace-pre-wrap bg-white p-4 rounded-md shadow text-sm text-foreground overflow-x-auto">
            {state.learningPaths}
          </pre>
        </div>
      )}
      {!state.success && state.message && !state.learningPaths && (
         <div className="mt-6 p-6 border-t border-destructive/30 bg-destructive/10 rounded-b-lg">
